# -*- coding: utf-8 -*-
"""00_create_label_data_Cerclis.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1OdfDZfH2YV64kKm1NKjccHyr5zFf9By7

# Creating Label Data

Read all instructions before running this code!

This notebook is for creating label data for federal (lower 48) Cerclis sites ONLY. It is a modified copy of the other Create Label Data codes.
...
This code adds 0s to Cerclis data. The Cerclis data already has a collection of 1s and 0s in it, and so this modified code maintains the original 1s and 0s while adding new ones. The following list includes changes from the original code:
1. No "intermediate" dataset is saved in the middle of the code, because the Cerclis data already exists and acts as the "intermediate". That entire section of code is removed.
2. Lines that assign an indicator to the original data are removed, as indicators are already assigned.
3. The section of code that creates 1s and 0s is modified to separate the original Cerclis data from the additional 0s.
...
file_path = "https://github.com/rachelmyoung/pollution-searching/raw/refs/heads/main/Data/Raw/CERCLIS_test_labels_lower48.csv"
...
output_path = "/content/drive/MyDrive/MOSAIKS/superfund_project_summer2026/Data/Intermediate/Cerclis" #where you want to save your final .csv
...
"""Section of code is removed here for Cerclis."""

drive_file_path = file_path
...
#This count confirms the lat/lon is still the same as the original Cerclis data, even with visual discrepancies
#Still use the left lat/lon for creating labels
unaltered_lat_count = pois_grid['lat_right'].isin(usa_pois_gdf['lat']).sum()
print("Number of unchanged latitudes is " + str(unaltered_lat_count))
...
## Lines commented out because Cerclis already has these columns
## Group by grid cell (using lat/lon) and fclass, then count
#pois_grid_count = pois_grid.groupby(
#    ["lat_left", "lon_left", "indicator"],
#    as_index=False,
#).unique_id.count()

# Rename the fclass column, drop unnecessary columns
pois_grid_count = pois_grid
pois_grid_count = pois_grid_count.rename(columns={
    "lat_left": "lat",
    "lon_left": "lon"
    })
pois_grid_count = pois_grid_count.drop(columns=['id','unique_id','cell_area','lat_rounded','index_right','lat_right','lon_right'])
...
print(pois_grid_count.tail()) #check: confirms we still have the original Cerclis 1s and 0s
...
#Split Cerclis 0s and 1s
labels_positive_Cerclis = pois_grid_count[pois_grid_count['indicator'] == 1].copy()
print(f"Loaded {len(labels_positive_Cerclis)} positive (1s) rows in Cerclis data.")

labels_negative_Cerclis = pois_grid_count[pois_grid_count['indicator'] == 0].copy()
print(f"Loaded {len(labels_negative_Cerclis)} negative (0s) rows in Cerclis data.")

# Create a unique 'lat_lon_key' for original pos and neg Cerclis values
labels_positive_Cerclis['lat_lon_key'] = labels_positive_Cerclis['lat'].astype(str) + '_' + labels_positive_Cerclis['lon'].astype(str)
labels_negative_Cerclis['lat_lon_key'] = labels_negative_Cerclis['lat'].astype(str) + '_' + labels_negative_Cerclis['lon'].astype(str)
...
# Get the list of keys that are already in the Cerclis data
positive_keys_Cerclis = labels_positive_Cerclis['lat_lon_key'].unique()
negative_keys_Cerclis = labels_negative_Cerclis['lat_lon_key'].unique()

# Combine positive and negative keys into a single list for filtering
all_Cerclis_keys = np.concatenate((positive_keys_Cerclis, negative_keys_Cerclis))

# Filter the full grid to find all rows that ARE NOT in the combined Cerclis keys list
labels_negative = usa_grid_gdf[~usa_grid_gdf['lat_lon_key'].isin(all_Cerclis_keys)].copy()
print(f"Found {len(labels_negative)} negative (0s) rows.")

# Logic confirmation: confirm no overlap between labels_negative and pois_grid_count
pois_grid_count_keys = pois_grid_count['lat'].astype(str) + '_' + pois_grid_count['lon'].astype(str)
labels_negative_keys = labels_negative['lat_lon_key']
overlap_count = labels_negative_keys.isin(pois_grid_count_keys).sum()
if overlap_count == 0:
    print("Confirmation: No overlap found between labels_negative and pois_grid_count.")
else:
    print(f"Warning: {overlap_count} overlapping points found between labels_negative and pois_grid_count.")

#Cleanup
del all_Cerclis_keys
...
sample_size = len(labels_positive_Cerclis) * (percent_negatives)
...
#labels_negative_neighbors = labels_positive_Cerclis.copy()
#labels_negative_neighbors['lon'] = (labels_negative_neighbors['lon'] - 0.02).round(3)

#ADDS ALL NEGATIVE LABELS TOGETHER
# If you intend to use shifted neighbors, switch the lines below.
...
# Turn the negative coordinates into actual Point geometries
...
# We use 'sjoin' to find which of our new negative points actually land inside a grid cell.
...
positive_keys = labels_positive_Cerclis['lat'].astype(str) + labels_positive_Cerclis['lon'].astype(str)
...
final_labels = pd.concat([
    pois_grid_count[['lat', 'lon', 'indicator']],
    final_negatives[['lat', 'lon', 'indicator']]
]).reset_index(drop=True)

# Check
print(f"Cerclis positives: {len(labels_positive_Cerclis)}")
print(f"Valid Negatives found (after sampling): {len(final_negatives)}")
print(f"Cerclis negatives: {len(labels_negative_Cerclis)}")
print(f"Final labels: {final_labels['indicator'].value_counts()}")
...
##Only Cerclis on a map
#pois_grid_count.explore()

##Only positives on a map
#labels_positive_Cerclis.explore()
...
file_suffix = file_suffix + "_" + str(date.today()) + "_Cerclis_plus_400pctnegs"
